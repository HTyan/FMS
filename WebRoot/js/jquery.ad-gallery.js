(function($) {
	$.fn.adGallery = function(options) {
		...
		if (options && options.slideshow) {
			settings.slideshow = $.extend(false, defaults.slideshow,
					options.slideshow);
		}
		;
		...
		findImages : function() {
			...
			thumbs.each(function(i) {
				...
				// Check if the thumb has already loaded
						...
						return false;
							...
							function() {
								if (!$(this).is('.ad-active')
										&& context.settings.thumb_opacity < 1) {
							...
							function() {
								if (!$(this).is('.ad-active')
									...
									$(this).find('img').fadeTo(300,
											context.settings.thumb_opacity);
								}
								;
							});
					var link = false;
					...
					} else if (thumb.attr('title')
							&& thumb.attr('title').length) {
